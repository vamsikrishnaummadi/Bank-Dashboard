...
import {errorHandler} from '../middlewares/error.js'; // Adjust the import according to your project structure
...
export const getInvestmentsByAccountNumber = async (req, res, next) => {
    ...
    try {
        ...
        if (!investment) {
            return next(errorHandler(404, 'Investment not found'));
        }
...
export const createInvestment = async (req, res, next) => {
    ...
    try {
        ...
        const transactionData = { amount: investment.amount, type: 'investment', accountNumber: investment.accountNumber, cardId : investment.cardId, invoice: "Invested Certain amount", description: investment.description };
...
export const deleteInvestments = async (req, res, next) => {
    ...
    try {
        ...
        if (!deletedInvestment) {
            return next(errorHandler(404, 'Investments not found'));
        }
...
export const getInvestmentStatistics = async (req, res, next) => {
    ...
    try {
        ...
        const totalAmountInvested = investments.reduce((sum, investment) => sum + investment.amount, 0);
        const numberOfInvestments = investments.length;
        const averageReturnPercentage = investments.reduce((sum, investment) => sum + investment.returnPercentage, 0) / numberOfInvestments;
